...
- **Educational Content**: An evidence-based education library with articles on periods, PCOS, endometriosis, and more. Articles have hero images displayed in both the Learn screen cards and the article detail view. Article data in `client/data/articles.ts`, images in `client/assets/images/articles/`. `ArticleCard` component supports both `imageSource` (require'd local assets) and `imageUri` (URL strings) for thumbnails and featured hero images. `ArticleDetailScreen` renders a hero image below the subtitle.
- **AI Health Assistant**: A conversational health companion ("Olanna") supporting English and South African languages. Features: conversation memory (last 10 messages sent as history for contextual follow-ups), animated typing indicator (3 pulsing dots), markdown-formatted responses via `MarkdownText` component (bold, bullets, headings, dividers), gradient user message bubbles (#E83E8C → #D633A6), glass assistant bubbles with chat-style rounded corners, timestamps on all messages, personalized welcome card with user's name and Olanna branding, prompt chips with Feather icons that disappear after first message, "New Chat" header button to reset conversation. Prompt catalog in `client/constants/promptCatalog.ts`. Backend endpoint `POST /api/assistant` (`server/assistantRoutes.ts`) accepts `promptId` or `freeText` with `userContext` and optional `history` array, uses structured prompt templates with user context interpolation, gpt-4.1-mini model at temperature 0.4 with max 1500 tokens, and appends a medical safety footer to every response. System prompt instructs warm "trusted older sister" tone with South African cultural warmth, markdown formatting, and shorter follow-up responses. Accessible from Profile screen. Six built-in prompt templates: phase_today, symptom_normal, late_period, cramps_help, pcos_basics, doctor_when.
...
- **Design System**: Token-based color system in `client/constants/colors.ts` with brand (#E83E8C primary, gradient #FF6A4D → #FF2F8E → #D633A6), neutral (warm cream #F8F6F4 base, textPrimary #2D1F2B, textSecondary #3D2838, textTertiary #5A4550), phase (Menstrual: soft pink #F2A2B8, Follicular: cool grey/lilac #CFCBD6, Ovulatory: warm peach #F2C9A2, Luteal: lavender #D7B3E7), and semantic (success/warning/danger/info) tokens. Text tokens darkened for WCAG AA readability on gradient/glass backgrounds. Helper utilities: `getPhaseGradient()`, `getPhaseColors()`, `getBrandGradient()`. Cycle components use tokens exclusively — no hardcoded hex values. Other screens use `AppGradient` component (`client/components/AppGradient.tsx`) for brand gradient backgrounds.
