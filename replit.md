...
- **Cycle Data Architecture**: Modular three-layer design: (1) `client/lib/cycleService.ts` — pure stateless calculation service with `computeCycleStatus()`, `generateCalendarMarkers()`, `predictFertileWindow()`, `computeCycleDay()`, and `getEffectiveLastPeriodStart()`; (2) `client/hooks/useCycleData.ts` — shared hook for CycleScreen + HomeScreen that loads profile + logs on focus and returns `CycleStatus`; (3) `client/hooks/useCalendarData.ts` — calendar-specific hook that generates per-day markers with period/fertile/phase/PMS predictions. All screens use these hooks for consistent, auto-refreshing data. `storage.ts` delegates to `cycleService.ts` via `calculateCycleDataWithLogs()`.
- **Cycle Synchronization**: `getEffectiveLastPeriodStart()` derives the real period start from flow logs (not just the profile's `lastPeriodStart`). All screens use the shared hooks to stay synchronized — logging a period on the Calendar immediately reflects on the Cycle wheel as Day 1 Menstrual. Period logs support editing (pre-filled flow/mood/notes, "Update" button), mood-only saves, flow toggle-off (tap to deselect), and deletion via "Remove this entry" button (`storage.removeDailyLog`). Symptom detail modal includes a "Clear this one" button for removing previously logged severity symptoms.
