...
- **Cycle Synchronization**: `getEffectiveLastPeriodStart()` and `calculateCycleDataWithLogs()` in `storage.ts` derive the real period start from flow logs (not just the profile's `lastPeriodStart`). All screens (CycleScreen, CalendarScreen, HomeScreen) use this to stay synchronized — logging a period on the Calendar immediately reflects on the Cycle wheel as Day 1 Menstrual. Period logs support editing (pre-filled flow/mood/notes, "Update" button), mood-only saves, flow toggle-off (tap to deselect), and deletion via "Remove this entry" button (`storage.removeDailyLog`). Symptom detail modal includes a "Clear this one" button for removing previously logged severity symptoms.
